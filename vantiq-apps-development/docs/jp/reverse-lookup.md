...
- [Source 関連](#Source関連)
  ...
  - [イベントが高負荷すぎてバッジが更新しなくなったが、件数を確認したい](#イベントが高負荷すぎてバッジが更新しなくなったが件数を確認したい)
- [App Builder 関連](#AppBuilder関連)
  ...
  - [DB でクレジットエラーが出るのを回避したい](#DBでクレジットエラーが出るのを回避したい)
- [構成管理関連](#構成管理関連)
...
### ひとつのSourceの定義でさまざまなWebサービスを呼び分けしたい
...
### イベントが高負荷すぎてバッジが更新しなくなったが、件数を確認したい<a id="イベントが高負荷すぎてバッジが更新しなくなったが件数を確認したい"></a>
Grafanaを使用する。
...
## App Builder関連<a id="AppBuilder関連"></a>
...
### 一定時間イベントが発生しない場合にアラートを出したい
[`missing`](https://dev.vantiq.co.jp/docs/system/apps/index.html#missing)タスクを使用します。missingは一定時間入力がない場合にイベントが発火します。

### 3つ以上のストリームデータを効率よく結合したい
1. [`join`](https://dev.vantiq.co.jp/docs/system/apps/index.html#join)を使います。
1. `AccumulateState`を使い、一連のイベントのプロパティを同一イベントにトランスポーズします。
...
### 一定時間経過すると自動的に古いレコードを消去したい
TypeのAdvanced設定で、`exipires after`を設定します。

### 複数のアプリケーションで使用できる共通なロジックを実装したい
...
### DB でクレジットエラーが出るのを回避したい<a id="DBでクレジットエラーが出るのを回避したい"></a>
...
### Sourceのリクエストヘッダー等に使うクレデンシャルを安全に保管したい  
