...
### イベントが高負荷すぎてバッジが更新しなくなったが、件数を確認したい
Grafanaを使用する。
...
### イベントに紐づけるマスタデータの欠損を、任意の内容のエラーでログ用のDBに送信して残したい
...
### JSONの配列になっている、もしくはJSONの配列を含む入力イベントを処理したい
...
### プロパティが入れ子になっている複雑な Type を定義したい
以下のいずれかの方法で行います。
...
### Vantiq から外部への API 呼び出しの数を制限したい
