...
1. AI カメラや位置情報取得アプリからのデータを受信する
   * プロトコルやバージョンアップによる仕様変更などに対応しやすいことが望ましい  

1. AI カメラから送信されるデータに対して必要な情報を追加する  
   * AI カメラが位置情報を持たないため Vantiq 側で保持しておき追加する  

1. 警備員の最新の位置情報を保存・更新する  
   * 通知へのタイムラグを減らすため、不審者検出時に警備員の位置情報を取得するのではなく予め Vantiq 側で保持しておく  

1. 通知が行われた AI カメラの位置と警備員の位置の比較  
   * 最も近い警備員を特定する  
...
③
アプリは Type の情報を取得して AI カメラからのストリームデータに追加する (Enrich)  
```sh
{
👉“camera_id”: "camera_001",
👉“lan”: 35.6864604,
...
```
② 位置を比較するロジックをプログラムした自作ファンクションをアプリから呼び出し  
...
③ パターン外の処理は独自に実装  
④ デバイスから送信されるストリームデータ  
⑤ ストリームデータに追加したい情報や、リアルタイムのステータスなどを保存
