...
- [Fitbit API](#fitbit-api)
  ...
  - [OAuth 2.0 PKCE 認証](#oauth-20-pkce-認証)
    ...
    - [【補足】VANTIQ のみで OAuth2.0 PKCE 認証 を用いた外部サービスとの連携が難しい理由](#補足vantiq-のみで-oauth20-pkce-認証-を用いた外部サービスとの連携が難しい理由)
      - [1. 認証コードと個人の紐づけが出来ない点](#1-認証コードと個人の紐づけが出来ない点)
      - [2. コールバックURLへのリダイレクトが直接出来ない点](#2-コールバックurlへのリダイレクトが直接出来ない点)
  - [環境構築](#環境構築)
  ...
  - [API アクセス](#api-アクセス)
    ...
    - [API の一例](#api-の一例)
      ...
      - [Get Temperature (Core) Summary by Date](#get-temperature-core-summary-by-date)
...
### 【補足】VANTIQ のみで OAuth2.0 PKCE 認証 を用いた外部サービスとの連携が難しい理由
#### 1. 認証コードと個人の紐づけが出来ない点
- OAuth 2.0 PKCE 認証時の `code_verifier` と `code_challenge` は認証を試みるごとに生成する必要があり、かつ、 `code_verifier` は後から再利用するため、認証を試みた個人と紐づけて一時的に保存しておく必要があります。  
- OAuth 2.0 PKCE 認証後、コールバックURLにリダイレクトする際に、個人を識別するためのパラメータなどの付与が出来ないため、Fitbit で発行された認証コードと個人の紐づけが出来ません。  

このため、まずは個人を管理するためにユーザーの認証やセッション情報などを管理する機能が VANTIQ または、外部サービスを用いて構築する必要が生じます。  

#### 2. コールバックURLへのリダイレクトが直接出来ない点
- コールバックURLへのリダイレクトについては、GETアクセスになるため、プロシージャ等への直接のリダイレクトは出来ません。
- Webサーバーや APIゲートウェイ等を用いて、POSTアクセスに直す必要があります。  
